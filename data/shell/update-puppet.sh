...
if [[ ! -f /.hip-stuff/update-puppet ]]; then
    if [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
        echo "Downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        wget --quiet --tries=5 --timeout=10 -O "/.hip-stuff/puppetlabs-release-${CODENAME}.deb" "http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        echo "Finished downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"

        dpkg -i "/.hip-stuff/puppetlabs-release-${CODENAME}.deb" >/dev/null
        ...
        touch /.hip-stuff/update-puppet
        echo "Created empty file /.hip-stuff/update-puppet"
    elif [ "${OS}" == 'centos' ]; then
