...
VAGRANT_CORE_FOLDER=$(cat "/.hip-stuff/vagrant-core-folder.txt")
...
if [ "${FOUND_GIT}" -ne '0' ] && [ ! -f /.hip-stuff/librarian-puppet-installed ]; then
...
if [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
    if [[ ! -f /.hip-stuff/librarian-base-packages ]]; then
        ...
        touch /.hip-stuff/librarian-base-packages
    fi
fi

if [ "${OS}" == 'ubuntu' ]; then
    if [[ ! -f /.hip-stuff/librarian-libgemplugin-ruby ]]; then
        ...
        touch /.hip-stuff/librarian-libgemplugin-ruby
    fi

    if [ "${CODENAME}" == 'lucid' ] && [ ! -f /.hip-stuff/librarian-rubygems-update ]; then
        ...
        touch /.hip-stuff/librarian-rubygems-update
    fi
fi

if [[ ! -f /.hip-stuff/librarian-puppet-installed ]]; then
    ...
    touch /.hip-stuff/librarian-puppet-installed
else
